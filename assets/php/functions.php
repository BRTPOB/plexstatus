...
$config_file_path = "/var/www/status/config.ini"; // Path to config file, replace the expression after the "=" sign. Don't forget to leave the ";" at the end of the line. You should place it outside of web root
...
//global $mylar_instances;
$mylar_instances = array();
if (isset($config['mylar'])) {
	$mylar_config_line_count = (count($config['mylar'], COUNT_RECURSIVE) - $mylar_config_lines);
	if ($mylar_config_line_count >= $mylar_config_lines) {
		for ($i=1; $i <= $mylar_config_line_count; $i++) {
			$num = ceil($i/$mylar_config_lines);
			$mylar_instances[$num][0] = $config['mylar']['server_name'][$num];
			$mylar_instances[$num][1] = $config['mylar']['local_port'][$num];
			$mylar_instances[$num][2] = $config['mylar']['URL'][$num];
			$mylar_instances[$num][3] = $config['mylar']['local_ip'][$num];
			$mylar_instances[$num][4] = $config['mylar']['username'][$num];
			$mylar_instances[$num][5] = $config['mylar']['password'][$num];
		}
	}
	else {
		$mylar_instances = NULL;
	}
}
...
{
	$du = getDiskspaceUsed("/") + getDiskspaceused("/home") + getDiskspaceUsed("/var") + getDiskspaceUsed("/opt") + getDiskspaceUsed("/media/backups") + getDiskspaceUsed("/media/Comics") + getDiskspaceUsed("/media/Movies") + getDiskspaceUsed("/media/n5200") + getDiskspaceUsed("/media/Newsgroups") + getDiskspaceUsed("/media/nzbfiles") + getDiskspaceUsed("/media/Seagate Backup Plus Drive") + getDiskspaceUsed("/media/downloads");
	
	$dts = disk_total_space("/") + disk_total_space("/home") + disk_total_space("/var") + disk_total_space("/opt") + disk_total_space("/media/backups") + disk_total_space("/media/Comics") + disk_total_space("/media/Movies") + disk_total_space("/media/n5200") + disk_total_space("/media/Newsgroups") + disk_total_space("/media/nzbfiles") + disk_total_space("/media/Seagate Backup Plus Drive") + disk_total_space("/media/downloads");
...
function makeDiskBars()
{
	...
	printDiskBar(getDiskspace("/"), "OS", disk_free_space("/"), disk_total_space("/"));
	printDiskBar(getDiskspace("/home"), "Home SSD", disk_free_space("/home"), disk_total_space("/home"));
	printDiskBar(getDiskspace("/var"), "var", disk_free_space("/var"), disk_total_space("/var"));
	printDiskBar(getDiskspace("/opt"), "opt", disk_free_space("/opt"), disk_total_space("/opt"));
	printDiskBar(getDiskspace("/media/Comics"), "Comics", disk_free_space("/media/Comics"), disk_total_space("/media/Comics"));
	printDiskBar(getDiskspace("/media/Movies"), "Movies", disk_free_space("/media/Movies"), disk_total_space("/media/Movies"));
	printDiskBar(getDiskspace("/media/n5200"), "n5200", disk_free_space("/media/n5200"), disk_total_space("/media/n5200"));
	printDiskBar(getDiskspace("/media/Newsgroups"), "Newsgroups", disk_free_space("/media/Newsgroups"), disk_total_space("/media/Newsgroups"));
	printDiskBar(getDiskspace("/media/nzbfiles"), "nzbfiles", disk_free_space("/media/nzbfiles"), disk_total_space("/media/nzbfiles"));
	printDiskBar(getDiskspace("/media/Seagate Backup Plus Drive"), "Seagate Backup Plus Drive", disk_free_space("/media/Seagate Backup Plus Drive"), disk_total_space("/media/Seagate Backup Plus Drive"));
	printDiskBar(getDiskspace("/media/downloads"), "downloads/Plex", disk_free_space("/media/downloads"), disk_total_space("/media/downloads"));
/*	printDiskBar(getDiskspaceErebor("/media/Erebor"), "Erebor", ($GLOBALS['ereborTotalSpace'] - getDiskspaceUsed("/media/Erebor")), $GLOBALS['ereborTotalSpace']);
	printDiskBar(getDiskspaceTV1("/media/Television"), "Narya", ($GLOBALS['televisionTotalSpace'] - getDiskspaceUsed("/media/Television")), $GLOBALS['televisionTotalSpace']);
	printDiskBar(getDiskspaceTV2("/media/Television 2"), "Nenya", ($GLOBALS['television2TotalSpace'] - getDiskspaceUsed("/media/Television 2")), $GLOBALS['television2TotalSpace']);
*/
}
...
/* function callTraktAPI()
{
    ...
    $request->setUrl('https://api-v2launch.trakt.tv/users/Offspring');
    $request->setMethod(HTTP_METH_GET);

    $request->setHeaders(array(
        'trakt-api-version' => '2',
//        'trakt-api-key' => 'c70dbe0b15d57f97e7baad16024ba0fd32a26ba9531a938f3b893d03e026b742',
	  'trakt-api-key' => '9f7ea270a3971b5d44e15b569bf21fd78d7402a69ea756a45eda4a786b748cdc',
...
//		$plexNewestXML = simplexml_load_file($network.':'.$plex_port.'/library/sections/1/newest'); //the number relates to how many libraries you have..i think..
		$plexNewestXML = simplexml_load_file($network.':'.$plex_port.'/library/sections/2/newest');
...
//		$movieArt = $mediaXML->Video['thumb'];
		$movieArt = $mediaXML->Video['grandparentThumb'];
		$movieYear = $mediaXML->Video['year'];
		$mediaShowTitle = $mediaXML->Video['grandparentTitle'];
		$seasonNumber = $mediaXML->Video['parentIndex'];
		$episodeNumber = $mediaXML->Video['index'];
		$mediaShowSummary = $mediaXML->Video['summary'];

		echo '<img src="plex.php?img='.urlencode($network.':'.$plex_port.$movieArt).'" alt="'.$movieTitle.'">';
		echo '<div class="carousel-caption">';
//		echo '<h3>'.$mediaShowTitle.' ('.$episodeNumber.')</h3>';
		echo '<h3>'.$mediaShowTitle.' (S'.$seasonNumber.'E'.$episodeNumber.')</h3>';
//		echo '<p>Summary</p>'
		echo '<p>'.$mediaShowSummary.'</p>';
		echo '</div>';
